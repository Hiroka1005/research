...
const int NP = 1024;               // Particle number.
...
const double RCHK = 2.3;
const double rcut = 1.4;
const double phiini = 0.850;
const double phimax = 0.850;
const double f_thresh = 1.e-13;
const double deltaphi_change = 1.0e-4;
const double deltaphi_observe = 1.0e-2;
const double gamma0 = 1.0e-7;
...
__global__ void eom_kernel_OS(double *x_dev, double *y_dev, double *fx_dev, double *fy_dev, double *L_dev, double *dt_dev, double *gamma_dev, double *gamma_dot_dev)
{
  int i_global = threadIdx.x + blockIdx.x * blockDim.x;
  double y_temp;
  ...
  {
    // move particles obey EoM
    x_dev[i_global] += (*gamma_dot_dev * y_dev[i_global] + fx_dev[i_global]) * dt_dev[0]; // x[i][0] += (gamma0*omega*cos(omega*t)*x[i][1]+f[i][0])*dt;
    y_dev[i_global] += fy_dev[i_global] * dt_dev[0];                                      // x[i][1] += f[i][1]*dt;
...
__global__ void FIRE_synth_dev_SS(double *vx_dev, double *vy_dev, double *fx_dev, double *fy_dev, double *power_dev, double *alpha_dev, int *FIRE_gate_dev, double *stress_dev, double *gamma_dot_dev)
{
  int i_global = threadIdx.x + blockIdx.x * blockDim.x;
  double f, v;
  if (i_global < NP)
...
__global__ void FIRE_reset_dev_SS(double *vx_dev, double *vy_dev, double *power_dev, double *alpha_dev, double *dt_dev, int *FIRE_param_gate_dev, double *gamma_dot_dev)
{
  int i_global = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void change_deltaphi(double *deltaphi_dev, double *phi_dev, double *pot_dev, int flg_dphi)
{
  if (*phi_dev >= phimax)
    *deltaphi_dev = -1.e-4;
  if (*deltaphi_dev < 0 && flg_dphi == 1)
    *deltaphi_dev = -1.e-5;
  if (*deltaphi_dev < 0 && flg_dphi == 2)
    *deltaphi_dev = -1.e-6;
  __syncthreads();
}
...
__global__ void shear_affine(double *x_dev, double *y_dev, double *delta_gamma_dev, double *gamma_dev, double *gamma_observe_dev)
{
  ...
  if (i_global == 0)
  {
    ...
    *gamma_observe_dev += *delta_gamma_dev;
    // printf("gamma_dev=%.8f,delta_gamma_dev=%.8f\n", gamma_dev, delta_gamma_dev);
  }
...
void FIRE_SS(double *x_dev, double *y_dev, double *vx_dev, double *vy_dev, double *fx_dev, double *fy_dev, double *a_dev, double *L_dev, int *list_dev, double *power_dev, double *alpha_dev, double *dt_dev, int *FIRE_gate_dev, int *FIRE_param_gate_dev, int *reduce_dev, int *remain_dev, double *gamma_dev, double *gamma_dot_dev, double *stress_dev)
{
  ...
  FIRE_synth_dev_SS<<<NB, NT>>>(vx_dev, vy_dev, fx_dev, fy_dev, power_dev, alpha_dev, FIRE_gate_dev, stress_dev, gamma_dot_dev);

  double sum_power = thrust::reduce(thrust::device_pointer_cast(power_dev), thrust::device_pointer_cast(power_dev + NB * NT), 0.0, thrust::plus<double>());
  cudaMemcpy(&power_dev[0], &sum_power, sizeof(double), cudaMemcpyHostToDevice);
  FIRE_reset_dev_SS<<<NB, NT>>>(vx_dev, vy_dev, power_dev, alpha_dev, dt_dev, FIRE_param_gate_dev, gamma_dot_dev);
  shear_affine_SS<<<NB, NT>>>(x_dev, y_dev, gamma_dot_dev, dt_dev);
}

void output_phiJ(double phiJ)
{
  char filename[128];
  ofstream file;
  if (deltaphi_observe > 0.0)
  {
    sprintf(filename, "phiJ_OS_ajam_dphi%1.1e_g%1.1e.dat", fabs(deltaphi_observe), gamma0);
  }
  else
  {
    sprintf(filename, "phiJ_OS_bjam_dphi%1.1e_g%1.1e.dat", fabs(deltaphi_observe), gamma0);
  }
...
void output_coord(double *x, double *y, double *a)
{
  static int count = 1;
  char filename[128];
...
void output_stress(int t, double p, double sxy0, double sxy, double gamma, double omega, int cycle)
{
  char filename[128];
  std::ofstream file;
  if (deltaphi_observe > 0.0)
...
void output_modulus(double G1, double G2, double omega, int cycle)
{
  char filename[128];
  ofstream file;
  if (deltaphi_observe > 0.0)
...
int main()
{
  double *x, *y, *vx, *vy, *x_dev, *y_dev, *x_cpdev, *y_cpdev, *vx_dev, *vy_dev, *dx_dev, *dy_dev, *a, *a_dev, *fx_dev, *fy_dev, *power_dev, *L_dev, *deltaphi_dev, deltaphi;
  double *pot_dev, *pressure_dev, *stress_dev, *pot, *pressure, *stress, *init_stress;
  double *dt_dev, *alpha_dev, *phi_dev, phi;
  double *gamma_dev, *gamma_observe_dev, *delta_gamma_dev, *gamma_dot_dev, gamma;
  double C, omega = 1.0e-5, G1 = 0.0, G2 = 0.0, t = 0.0, dt = 1.0e-1, tmax;
  int *list_dev, *map_dev, *gate_dev, *remain_dev, *reduce_dev;
  int *FIRE_gate_dev, FIRE_gate, *FIRE_param_gate_dev;
  int clock = 0, step = 0, out = 10000, flg_dphi = 0;
  ...
  init_stress = (double *)malloc(NB * NT * sizeof(double));
  ...
  cudaMalloc((void **)&x_cpdev, NB * NT * sizeof(double));
  cudaMalloc((void **)&y_cpdev, NB * NT * sizeof(double));
  ...
  // 　AQS shear training
  // cout << "start shear training" << endl;
  for (int cnt_train = 1; cnt_train <= 100; cnt_train++)
  {
    while (gamma < gamma0)
    {
      init_scalar_kernel<<<1, 1>>>(delta_gamma_dev, 1.0e-7);
  ...
  for (;;)
  {
    ...
    for (;;)
    {
      ...
      copy_kernel<<<NB, NT>>>(x_cpdev, y_cpdev, x_dev, y_dev);
    }
    if (pot[0] / NP < 1.e-16 && flg_dphi >= 2)
    {
      // cout << "phiJ=" << phi << endl;
      output_phiJ(phi);
      break;
    }
    else if (pot[0] / NP < 1.e-16 && flg_dphi <= 1)
    {
      flg_dphi++;
      // rewind configurations
      copy_kernel<<<NB, NT>>>(x_dev, y_dev, x_cpdev, y_cpdev);
      phi -= deltaphi;
      L = sqrt(M_PI * (1.0 * 1.0 + 1.4 * 1.4) * (double)NP / (8. * phi));
      ...
      // decrease |dphi|
      /*cout << "decrease |dphi|" << endl;
      cout << "|dphi|=" << abs(deltaphi) << endl;
      cout << " | " << endl;
      cout << " V " << endl;*/
      change_deltaphi<<<1, 1>>>(deltaphi_dev, phi_dev, pot_dev, flg_dphi);
      cudaMemcpy(&deltaphi, deltaphi_dev, sizeof(double), cudaMemcpyDeviceToHost);
      // cout << "|dphi|=" << abs(deltaphi) << endl;
    }
  ...
  for (double dphi = 0.0; abs(dphi) < abs(deltaphi_observe); dphi += deltaphi_change)
  {
    ...
    for (;;)
    {
      ...
      if (FIRE_gate == 1)
      {
        ...
        add_reduction_array(stress_dev, reduce_dev, remain_dev);
        add_reduction_array(pressure_dev, reduce_dev, remain_dev);
        cudaMemcpy(init_stress, stress_dev, NB * NT * sizeof(double), cudaMemcpyDeviceToHost);
        cudaMemcpy(pressure, pressure_dev, NB * NT * sizeof(double), cudaMemcpyDeviceToHost);
        cudaMemcpy(&phi, phi_dev, sizeof(double), cudaMemcpyDeviceToHost);

        /*cout << "phi= " << phi << "\t"
             << "dphi= " << dphi << "\t"
             << "pressure= " << pressure[0] << endl;*/
        clock = 0;
        break;
      }
    }
  ...
  while (omega < 1.0e-1 + 0.001)
  {
    ...
    for (int cycle = 1; cycle <= 10; cycle++)
    {
      while (t < tmax)
      {
        t += dt;
        step++;
        init_scalar_kernel<<<1, 1>>>(gamma_dev, gamma0 * sin(omega * t));
        init_scalar_kernel<<<1, 1>>>(gamma_dot_dev, gamma0 * omega * cos(omega * t));
        eom_kernel_OS<<<NB, NT>>>(x_dev, y_dev, fx_dev, fy_dev, L_dev, dt_dev, gamma_dev, gamma_dot_dev);
        list_auto_update(L_dev, x_dev, y_dev, vx_dev, vy_dev, dx_dev, dy_dev, dt_dev, M, map_dev, gate_dev, list_dev, gamma_dev);
        ...
        // calc modulus (sympthon method)
        if (step % 2 == 0)
        { // even term
          G1 += C * stress[0] * sin(omega * t) * 2.0;
          G2 += C * stress[0] * cos(omega * t) * 2.0;
        }
        else
        { // odd term
          G1 += C * stress[0] * sin(omega * t) * 4.0;
          G2 += C * stress[0] * cos(omega * t) * 4.0;
        }
        if (step == out)
        {
          /*cout << "t=" << t << "\t"
  ...
  cudaFree(L_dev);
  cudaFree(phi_dev);
  cudaFree(deltaphi_dev);
  cudaFree(delta_gamma_dev);
  cudaFree(gamma_dev);
  cudaFree(gamma_dot_dev);
  cudaFree(gamma_observe_dev);
  cudaFree(gate_dev);
  cudaFree(FIRE_gate_dev);
  cudaFree(FIRE_param_gate_dev);
  cudaFree(remain_dev);
  cudaFree(reduce_dev);
  cudaFree(list_dev);
  cudaFree(map_dev);
