...
void output_coord(double (*x)[dim], double *a, double gamma)
{
  char filename[128];
  std::ofstream file;
  if (dphi_fixed > 0.0)
  {
    sprintf(filename, "coord_AQS_ajam_dphi%1.1e_strain%1.4e.dat", fabs(dphi_fixed), gamma);
  }
  else
  {
    sprintf(filename, "coord_AQS_bjam_dphi%1.1e_strain%1.4e.dat", fabs(dphi_fixed), gamma);
  }
  file.open(filename);
  for (int i = 0; i < N1 + N2; i++)
  {
...
void output_phiJ(double phi)
{
  ...
  if (dphi_fixed > 0.0)
  {
    sprintf(filename, "phiJ_AQS_ajam_dphi%1.1e.dat", fabs(dphi_fixed));
  }
  else
  {
    sprintf(filename, "phiJ_AQS_bjam_dphi%1.1e.dat", fabs(dphi_fixed));
  }
...
void output_P_and_sxy(double p, double sxy0, double sxy, double gamma)
{
  char filename[128];
  std::ofstream file;
  if (dphi_fixed > 0.0)
  {
    sprintf(filename, "stress_ajam_dphi%1.1e.dat", fabs(dphi_fixed));
  }
  else
  {
    sprintf(filename, "stress_bjam_dphi%1.1e.dat", fabs(dphi_fixed));
  }
  file.open(filename, std::ios::app);
  file << std::setprecision(12) << gamma << "\t" << p << "\t" << sxy - sxy0 << std::endl;
  file.close();
}
...
int main()
{
  ...
  if (dphi_fixed > 0.0)
  {
    ...
    {
      affine_arithmetic(x, &L, &phi, 1.0e-5);
      list_verlet(list, x, 0.0, L);
  ...
  while (gamma <= 1.0e-3)
  {
    ...
    output_P_and_sxy(p, sxy0, sxy, gamma);
    // std::cout<<"strain="<<gamma<<"\t"<<"stress="<<sxy-sxy0<<std::endl;
  }
